...
Capturer *capturer_create(Display *dpy, Window root, int w, int h) {
  ...
  c->dpy = dpy;
  c->root = RootWindow(dpy, DefaultScreen(dpy));
  c->width = w;
  c->height = h;
...
void debug_create(int w, int h) {
  ...
  dbg->win = XCreateSimpleWindow(dbg->dpy, DefaultRootWindow(dbg->dpy), 0, 0, w,
                                 h, 0, BlackPixel(dbg->dpy, screen),
  ...
  dbg->img = XShmCreateImage(dbg->dpy, DefaultVisual(dbg->dpy, screen),
                             DefaultDepth(dbg->dpy, screen), ZPixmap, NULL,
                             &dbg->shm, w, h);

  dbg->shm.shmid =
      shmget(IPC_PRIVATE, dbg->img->bytes_per_line * h, IPC_CREAT | 0777);
...
void debug_show(DirtyFrame *frame) {
  ...
  for (int row = 0; row < frame->h; row++) {
    memcpy(dbg->img->data + (frame->y + row) * dbg->img->bytes_per_line +
               frame->x * 4,
           frame->pixels + row * frame->stride, frame->w * 4);
  }

  XShmPutImage(dbg->dpy, dbg->win, dbg->gc, dbg->img, 0, 0, 0, 0, dbg->w,
               dbg->h, False);
  XFlush(dbg->dpy);
}
